...
const enabled = configs.osmAuth && configs.osmAuth.oauth_consumer_key && configs.osmAuth.oauth_secret;
...
export default {
  ...
  load: function (callback) {
    if (!auth.authenticated()) {
      auth.authenticate(function(err) { //check err param exists
  ...
  delete: function (query, callback) {
    if (!auth.authenticated()) return callback(new Error("must be logged in to delete a synced query"));
    query = {
      name: query,
      deleteMe: true
    };
...
function loadQueries(callback) {
  auth.xhr({
    ...
    if (cnt_elem = res.querySelector('preference[k="'+configs.appname+'_query-count"]'))
      pref_count = +cnt_elem.getAttribute("v");

    var result = [];
    for (var i=0; i<pref_count; i++) {
      var first_chunk = res.querySelector('preference[k="'+configs.appname+'_query_'+i+'_0"]').getAttribute('v').split("&");
...
function saveQuery(new_query, callback) {
  loadQueries(function(err, existing_queries, dom) {
    ...
    var existing = preferences.querySelectorAll('preference[k^="'+configs.appname+'_query"]')
