function osmTable(pContainer, showIDs, showType, showGeometry) {
  if (showIDs == null) { showIDs = true; }
  if (showType == null) { showType = true; }
  if (showGeometry == null) { showGeometry = true; }
  ...
  this.generateTable = function generateTable() {
    ...
    for (var i=0; i<this.value.length; i++) {
      for (var j=0; j<this.value[i].features.length; j++) {
        ...
        var fixedColumns = {};
        //the fixed columns:
        fixedColumns = {"__id__": feature.properties.id,
    ...
    var fields = [];
    fields.push('__id__'); this.columns.push('osm-id');
    fields.push('__type__'); this.columns.push('type');
    fields.push('__geo__'); this.columns.push('geometry');
    ...
    //visibility of IDs, Type and geometry:
    if (!showIDs) { 
      $(this.deletedColumns).append('<option>osm-id</option>');
      this.datatable.fnSetColumnVis(0, false, false);
    }
    if (!showType) {
      $(this.deletedColumns).append('<option>type</option>');
      this.datatable.fnSetColumnVis(1, false, false);
    }
    if (!showGeometry) {
      $(this.deletedColumns).append('<option>geometry</option>');
      this.datatable.fnSetColumnVis(2, false, false);
    }
